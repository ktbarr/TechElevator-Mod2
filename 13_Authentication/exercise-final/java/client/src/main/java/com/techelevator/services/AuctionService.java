...
public class AuctionService {
    ...
    public Auction getOne(int id) throws AuctionServiceException {
        ...
        try {
            auction = restTemplate.exchange(BASE_URL + "auctions/" + id, HttpMethod.GET, makeAuthEntity(), Auction.class).getBody();
        } catch (RestClientResponseException ex) {
            throw new AuctionServiceException(ex.getRawStatusCode() + " : " + ex.getResponseBodyAsString());
        }
        return auction;
    ...
    public Auction[] getByTitle(String title) throws AuctionServiceException {
        Auction[] auctions = null;
        try {
            auctions = restTemplate.exchange(BASE_URL + "auctions?title_like=" + title, HttpMethod.GET, makeAuthEntity(), Auction[].class).getBody();
        } catch (RestClientResponseException ex) {
            throw new AuctionServiceException(ex.getRawStatusCode() + " : " + ex.getResponseBodyAsString());
        }
        return auctions;
    ...
    public Auction[] getByPrice(double price) throws AuctionServiceException {
        Auction[] auctions = null;
        try {
            auctions = restTemplate.exchange(BASE_URL + "auctions?currentBid_lte=" + price, HttpMethod.GET, makeAuthEntity(), Auction[].class).getBody();
    ...
    /**
     * Creates a new {HttpEntity} with the `Authorization: Bearer:` header and an auction request body
     * @param auction
     * @return
     */
