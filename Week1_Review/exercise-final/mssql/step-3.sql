...
select site_id, campground_id, site_number, max_occupancy, accessible, max_rv_length, utilities from site where utilities = 1 order by max_rv_length desc;

-- select total number of sites that have utilities hook up
select count(site_id) from site where utilities = 1;
...
select reservation_id, site_id, name, from_date, to_date from reservation where from_date BETWEEN '2020-09-01' AND '2020-09-30';
