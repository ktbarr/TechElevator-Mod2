...
public class JDBCCityDAO implements CityDAO
{
	...
	public City findCityById(long id)
	{
		...
		// we are only expecting a single city to be returned
		// so no need to loop, just check if one was found
		if (results.next())
		{
	...
	public List<City> findCityByCountryCode(String countryCode)
	{
		...
		String sqlFindCityByCountryCode = "SELECT id "
										+ "		, name "
										+ "		, countrycode "
										+ "		, district "
										+ "		, population " 
	...
	{
		String nextIdQuery = "SELECT nextval('seq_city_id') AS id;";
